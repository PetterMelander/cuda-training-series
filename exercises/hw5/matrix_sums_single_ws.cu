...
__global__ void row_sums(const float *A, float *sums, size_t ds)
{
  ...
  int warpId = (threadIdx.x + blockDim.x * blockIdx.x) / warpSize;
  if (warpId >= ds)
    return;

  // do a warp stride loop to reduce row to 32 values
  int lane = threadIdx.x % warpSize;
  float sum = 0.0f;
  for (int col = lane; col < ds; col += warpSize)
  {
    sum += A[col + ds * warpId];
  }

  // do a single warp shuffle reduction to reduce row to 1 value
...
int main()
{
  ...
  for (size_t i = 0; i < DSIZE * DSIZE; i++) // initialize matrix in host memory
    h_A[i] = 1.0f;
  ...
  row_sums<<<32 * (DSIZE + block_size - 1) / block_size, block_size>>>(d_A, d_sums, DSIZE);
